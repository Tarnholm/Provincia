// Run the same aligned diff between save_1.2 (baseline) and save_4.2 (diplomat-moved).
// This isolates the diplomat-move signal cleanly.
...
console.log(`A=save_1.2 (${A.length}B)  B=save_4.2 (${B.length}B)  ΔlenB-A=${B.length-A.length}`);
...
for (const e of inserts) {
  if ((e.type === 'INS_A' || e.type === 'INS_B') && e.len >= 4) {
...
// 1-byte insert/delete counts
const oneB = inserts.filter(e => (e.type === 'INS_A' || e.type === 'INS_B') && e.len === 1);
console.log(`\n1-byte inserts/deletes: ${oneB.length} (${oneB.filter(e => e.type === 'INS_B').length} B, ${oneB.filter(e => e.type === 'INS_A').length} A)`);
