// dig-gap619-1.js — investigate the 619 KB unclaimed gap at 0x61c47..0xf8fd2.
//
// Session 54 target: largest unclaimed range in save_1.2.sav per cover.js.
// Goal: identify structural pattern (records, padding, ascii, magic).
...
// ---- 1. Hexdump first 1 KB of gap --------------------------------------
console.log("\n=== First 256 bytes of gap (hex) ===");
for (let row = 0; row < 16; row++) {
  const base = GAP_START + row * 16;
  const hex = [];
  const ascii = [];
  for (let c = 0; c < 16; c++) {
    const b = buf[base + c];
...
// ---- 3. ASCII strings ------------------------------------------------
console.log("\n=== ASCII strings >= 6 chars (first 40) ===");
const strs = [];
let cur = "";
let curStart = -1;
for (let i = GAP_START; i < GAP_END; i++) {
  ...
  if (b >= 0x20 && b < 0x7f) {
    ...
    if (cur.length >= 6) strs.push({ off: curStart, s: cur });
    cur = ""; curStart = -1;
  }
}
if (cur.length >= 6) strs.push({ off: curStart, s: cur });
console.log(`  total strings >= 6 chars: ${strs.length}`);
for (let i = 0; i < Math.min(40, strs.length); i++) {
  const s = strs[i];
  console.log(`  0x${s.off.toString(16).padStart(8, "0")}  (${s.s.length}) ${JSON.stringify(s.s).slice(0, 100)}`);
}

// ---- 4. Look for known magic markers --------------------------------
const MAGICS = [
  { name: "ff 0a af f0 (faction)", b: Buffer.from([0xff, 0x0a, 0xaf, 0xf0]) },
  { name: "f0 0a af f0 (RLE/secondary)", b: Buffer.from([0xf0, 0x0a, 0xaf, 0xf0]) },
  { name: "0a 07 (save magic)", b: Buffer.from([0x0a, 0x07]) },
  { name: "section taw {selfPtr,size}", b: null }, // structural, scanned below
  { name: "ff ff ff ff (-1 sentinel)", b: Buffer.from([0xff, 0xff, 0xff, 0xff]) },
];
console.log("\n=== Magic marker hits in gap ===");
...
// ---- 6. Stride detection --------------------------------------------
// Sample autocorrelation at strides 16, 20, 24, 32, 40, 48, 64, 80, 96, 128, 256
console.log("\n=== Stride autocorrelation (lower = more periodic) ===");
const SAMPLE = Math.min(len, 200000);
const strides = [8, 12, 16, 20, 24, 28, 32, 40, 48, 56, 64, 80, 96, 100, 128, 160, 192, 256, 267, 308, 512];
...
// ---- 7. Quick look at first 32 bytes after gap start (any header?) --
console.log("\n=== Read potential header u32 fields at gap start ===");
for (let off = 0; off < 64; off += 4) {
  const u = buf.readUInt32LE(GAP_START + off);
  console.log(`  +${off.toString().padStart(2)}  u32=0x${u.toString(16).padStart(8, "0")}  dec=${u}`);
}

// ---- 8. Check whether the gap is mostly zero pages -----------------
// Count long zero runs >= 64
