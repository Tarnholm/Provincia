// dig-gap167-1.js — investigate the 167 KB unclaimed gap at 0x44e2..0x2d15a.
//
// Session 57 target: third-biggest unclaimed range in save_1.2.sav per cover.js.
// Sits between toggle_fow flag (single byte at 0x44e2) and the 214 KB sparse
// slot table at 0x2d4a9 (decoded session 56).
//
// Hypotheses:
//   1. Battle log — per-battle record of who fought whom, outcome
//   2. Faction reference / index table — 239 entries with faction metadata
//   3. Message log — UI text messages persisted in order
//   4. Event history — past 100 turns of major events
//   5. Lua state preamble — RTW Lua engine state for current campaign
...
// ---- 1. Hex dump first 1 KB and last 1 KB ----------------------------
...
hexdump("First 1 KB of gap", GAP_START, 64);
hexdump("Last 1 KB of gap",  GAP_END - 1024, 64);
...
// ---- 3. ASCII strings ------------------------------------------------
console.log("\n=== ASCII strings >= 5 chars (first 60) ===");
const strs = [];
let cur = "";
let curStart = -1;
...
for (let i = 0; i < Math.min(60, strs.length); i++) {
  const s = strs[i];
...
const SAMPLE = Math.min(len, 160000);
const strides = [4, 8, 12, 16, 20, 24, 28, 32, 40, 48, 56, 64, 80, 96, 100, 128, 160, 192, 239, 256, 267, 308, 512, 700, 717, 1024];
...
// ---- 10. Faction-id u32 scan (small ints in range 0..239) -----------
console.log("\n=== u32 small-int distribution (potential faction ids) ===");
const idHist = new Uint32Array(256);
let idTotal = 0;
for (let i = GAP_START; i < GAP_END - 4; i += 4) {
...
console.log(`  u32 values <= 239: ${idTotal} (${(idTotal*4/len*100).toFixed(2)}% of bytes)`);
const idSorted = [...idHist].map((c,b)=>({b,c})).sort((a,b)=>b.c-a.c).slice(0,12);
for (const e of idSorted) {
  if (e.c === 0) continue;
  console.log(`  id ${e.b.toString().padStart(3)}  count=${e.c}`);
}

// ---- 11. Repeating 4-byte pattern (top occurring u32 values) --------
console.log("\n=== Most repeated u32 values (aligned, top 16) ===");
const u32Map = new Map();
for (let i = GAP_START; i < GAP_END - 4; i += 4) {
  const u = buf.readUInt32LE(i);
  u32Map.set(u, (u32Map.get(u) || 0) + 1);
}
const u32Sorted = [...u32Map.entries()].sort((a,b)=>b[1]-a[1]).slice(0,16);
for (const [u, c] of u32Sorted) {
  console.log(`  0x${u.toString(16).padStart(8,"0")}  count=${c}`);
}
