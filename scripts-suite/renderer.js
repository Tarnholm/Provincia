...
The <b>bump rule</b> adds extra tier requirements to higher-level buildings.` },
    ...
    { section: 'Bump Exception Levels', desc: 'Settlement levels a bump exception may apply to. Remove a level (e.g. large_town) and settlements of that size always take the full bump — both the per-chain rules above AND the global fertility exception stop applying to them. Default lists every level, so nothing changes until you prune it.',
      variable: 'BUMP_EXCEPTION_LEVELS', type: 'set', label: 'Levels an exception may apply to' },
