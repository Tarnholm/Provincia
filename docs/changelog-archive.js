// Archived changelog entries (moved out of the app bundle 2026-07-16).
// src/changelog.js keeps only the last ~5 versions — WelcomeScreen parses the
// whole module on every post-update launch and it had grown to 827KB.
// This file is NOT imported by anything; it exists so the history stays
...
const CHANGELOG_ARCHIVE = [
  ...
  },
  {
    ...
    items: [
      ...
      { type: "fix", text: "**Dead characters no longer mis-tagged to phantom 'rebel' factions.** Long-dead family members in a save's obituary region were being attributed to emergent 'rebel' factions (the tag actually came from a nearby UI banner asset, not the character) — the same deceased person could even show under two different rebel factions on consecutive turns. Those dead records are now correctly left unattributed." },
      { type: "fix", text: "**~140 settlements were being dropped from saves.** The settlement scanner only recognised settlements whose record ended in a particular byte pattern, silently skipping roughly a tenth of all settlements (including Rome, Athens, Syracuse) — which orphaned their garrisons and the field units standing in their regions, so on long campaigns only ~60% of units resolved to an owner. The scanner now reads the record correctly: settlements (and their buildings, garrisons, and units) resolve fully — unit ownership goes to ~100% on the test saves. Settlements stored under a region alias (e.g. Rome's 'Roma') now resolve too, and naval units are no longer mistaken for unattributed land units." },
    ],
  },
  {
    version: "0.9.767",
...
export default CHANGELOG_ARCHIVE;
