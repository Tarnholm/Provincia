...
const CHANGELOG = [
  {
    ...
    items: [
      ...
      { type: "fix", text: "**Army Setup and Starting Populations now respect \"export instead of overwrite\".** They wrote straight into the live mod whatever that setting said. Accented characters in an ANSI descr_strat also survive an edit now, instead of turning into replacement marks for good." },
      { type: "feature", text: "**Extinction Watch (Tools menu).** A faction is destroyed when its last living male family member dies, however much land it holds. This lists every faction by its living adult males at campaign start — leader and heir, boys and when the eldest comes of age, lines where everyone is sixty or older, and lines standing on a single tile. On RIS, eight factions start with exactly one; the Mauryans have Ashoka and two boys. Click a tier to filter, double-click a faction to focus it on the map." },
